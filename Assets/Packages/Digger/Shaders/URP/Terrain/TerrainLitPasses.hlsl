...
void NormalMapMix(float3 positionWS, float3 normal, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, inout half3 mixedNormal)
{
    #if defined(_NORMALMAP)
        float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
        half3 nrm = 0.0f;
        nrm += splatControl.r * UnpackNormalScale(SampleTriplanar(splatControl.r, positionWS, normal, _Normal0, sampler_Normal0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(SampleTriplanar(splatControl.g, positionWS, normal, _Normal1, sampler_Normal0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(SampleTriplanar(splatControl.b, positionWS, normal, _Normal2, sampler_Normal0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(SampleTriplanar(splatControl.a, positionWS, normal, _Normal3, sampler_Normal0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw), _NormalScale3);
...
void SplatmapMix(float3 positionWS, float3 normal, float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    diffAlbedo[0] = SampleTriplanar(splatControl.r, positionWS, normal, _Splat0, sampler_Splat0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw);
    diffAlbedo[1] = SampleTriplanar(splatControl.g, positionWS, normal, _Splat1, sampler_Splat0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw);
    diffAlbedo[2] = SampleTriplanar(splatControl.b, positionWS, normal, _Splat2, sampler_Splat0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw);
    diffAlbedo[3] = SampleTriplanar(splatControl.a, positionWS, normal, _Splat3, sampler_Splat0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw);
    ...
    NormalMapMix(positionWS, normal, uvSplat01, uvSplat23, splatControl, mixedNormal);
}
...
void ComputeMasks(half4 splatControl, out half4 masks[4], half4 hasMask, Varyings IN)
{
    masks[0] = 0.5h;
    masks[1] = 0.5h;
    masks[2] = 0.5h;
...
#ifdef _MASKMAP
    float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
    masks[0] = lerp(masks[0], SampleTriplanar(splatControl.r, IN.positionWS, IN.normal.xyz, _Mask0, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.x);
    masks[1] = lerp(masks[1], SampleTriplanar(splatControl.g, IN.positionWS, IN.normal.xyz, _Mask1, sampler_Mask0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTriplanar(splatControl.b, IN.positionWS, IN.normal.xyz, _Mask2, sampler_Mask0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw), hasMask.z);
    masks[3] = lerp(masks[3], SampleTriplanar(splatControl.a, IN.positionWS, IN.normal.xyz, _Mask3, sampler_Mask0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw), hasMask.w);
#endif
...
#endif
{
    ...
    half4 masks[4];
    ComputeMasks(splatControl, masks, hasMask, IN);
