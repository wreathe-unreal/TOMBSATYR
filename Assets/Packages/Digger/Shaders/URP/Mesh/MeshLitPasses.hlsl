...
#ifdef _MASKMAP
    masks[0] = lerp(masks[0], SampleTriplanar(splatControl.r, IN.positionWS, IN.normal.xyz, _Mask0, sampler_Mask0, _Splat0_ST.xy, _Splat0_ST.zw), hasMask.x);
    masks[1] = lerp(masks[1], SampleTriplanar(splatControl.g, IN.positionWS, IN.normal.xyz, _Mask1, sampler_Mask0, _Splat1_ST.xy, _Splat1_ST.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTriplanar(splatControl.b, IN.positionWS, IN.normal.xyz, _Mask2, sampler_Mask0, _Splat2_ST.xy, _Splat2_ST.zw), hasMask.z);
    masks[3] = lerp(masks[3], SampleTriplanar(splatControl.a, IN.positionWS, IN.normal.xyz, _Mask3, sampler_Mask0, _Splat3_ST.xy, _Splat3_ST.zw), hasMask.w);
#endif
...
#endif
{
    ...
    float mb = min(IN.bary.x, min(IN.bary.y, IN.bary.z));
    mb = saturate(mb * _BaryContrast);
    
    // now blend the normal
    float3 normal = abs(lerp(IN.normal.xyz, flatNormal, mb));

    SplatmapMix(IN.positionWS, normal, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
