...
#define SampleResultsTriplanar(i, mask)                                                                         \
    ...
    float3 flatNormal = normalWS;
#else
    float3 dx = ddx(positionWS);
    float3 dy = ddy(positionWS);
    float3 crossdxdy = cross(dy, dx);
    float3 flatNormal = normalize(crossdxdy == 0 ? normalWS : crossdxdy);
#endif
...
void TerrainLitDebug(float2 uv, inout float3 baseColor)
{
    ...
    if (_DebugMipMapModeTerrainTexture == DEBUGMIPMAPMODETERRAINTEXTURE_CONTROL)
        baseColor = GetTextureDataDebug(_DebugMipMapMode, uv, _Splat0, _Splat0_TexelSize, _Splat0_MipInfo, baseColor);
